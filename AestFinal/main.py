    ...
    while lista_aberta:
        ...
        for movimento in vizinhos:
            ...
            if (0 <= posicao_vizinha[0] < len(mapa)) and (0 <= posicao_vizinha[1] < len(mapa[0])):
                ...
                if mapa[posicao_vizinha[0]][posicao_vizinha[1]] == '■':
                    continue
...
def ler_mapa_arquivo(nome_arquivo):
    with open('AestFinal\mapa.txt') as arquivo:
...
custo_terreno_map = {
    'A': 1,
    '■': float('inf'),  # Obstáculos não têm custo de passagem
    'T': 3,
    'G': 5,
    'P': 10
}
...
# Uso:
mapa = ler_mapa_arquivo('mapa.txt')
inicio = (0, 0)  # Ponto inicial pode ser ajustado conforme necessário
paradas = [(1, 2), (2, 5), (3, 8), (9, 9)]
fim = (9, 4)  # Ponto final ajustado
